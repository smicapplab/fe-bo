<script>
	...
	import { aoiDocFormSchema } from '$lib/schemas/document';
	...
	const form = superForm(
		{},
		{
			dataType: 'json',
			// @ts-ignore
			validators: zodClient(aoiDocFormSchema),
			onSubmit: () => {
				isLoading = true;
			},
			onResult: (result) => {
...
<div class="rounded-lg border p-4">
	<h3 class="text-lg font-bold">Articles of Incorporation and ByLaws</h3>
								...
								<Popover.Content class="w-auto p-0" side="top">
									<AdvancedCalendar
										value={$formData.registrationDate
											...
											: null}
										minValue={currentDate.subtract({ years: 10 })}
