<script>
	...
	import { dtiDocFormSchema } from '$lib/schemas/document';
	...
	const form = superForm(
		{},
		{
			dataType: 'json',
			// @ts-ignore
			validators: zodClient(dtiDocFormSchema),
			onSubmit: () => {
				isLoading = true;
			},
			onResult: (result) => {
...
<div class="rounded-lg border p-4">
	...
	<p class="text-sm text-gray-700">Applicable only to Sole Proprietorship</p>
	{#if isLoading}
