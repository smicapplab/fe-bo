<script>
	...
	import { secDocFormSchema } from '$lib/schemas/document';

	export let docType = 'SEC';
	...
	const form = superForm(
		{},
		{
			dataType: 'json',
			// @ts-ignore
			validators: zodClient(secDocFormSchema),
			onSubmit: () => {
				isLoading = true;
			},
			onResult: (result) => {
...
<div class="rounded-lg border p-4">
	<h3 class="text-lg font-bold">
		...
		{:else}
			SEC Certificate of Amendment & Amendment Files (if applicable)
		{/if}
	</h3>
	<p class="text-sm text-gray-700">Upload all amendment files if applicable</p>
	{#if isLoading}
