<script>
	...
	const columns = table.createColumns([
		...
		table.column({
			...
			header: 'Company Type',
			cell: ({ value }) => (value ? getCompanyLabelByValue(value) : '-')
		}),
		table.column({
			accessor: 'contact',
			header: 'Contact',
		...
		table.column({
			accessor: 'status',
			header: 'Status',
			cell: ({ value }) => (value ? getStatusLabelByValue(value) : '-')
		}),
		table.column({
			accessor: 'createdAt',
				...
				<Table.Body {...$tableBodyAttrs}>
					{#each $pageRows as row (row.id)}
						<Subscribe rowAttrs={row.attrs()} let:rowAttrs>
							<Table.Row {...rowAttrs} on:click={() => openIssuer(row.original)} class="cursor-pointer">
